...
class KPICalculator:
    ...
    def calculate_order_profitability(order_id):
        ...
        base_cost = float(order.total_cost or 0)
        
        # Add rework costs
        rework_costs = ReworkLog.objects.filter(order=order).aggregate(
            material=models.Sum('extra_material_cost'),
            labor=models.Sum('extra_labor_cost')
        )
        
        rework_total = float(rework_costs['material'] or 0) + float(rework_costs['labor'] or 0)
