...
from .serializers import (
    ...
    EmployeeEfficiencySerializer, MachineSettingsSerializer
)
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            steps = [
                {"step_name": "queue", "assigned_to": settings.default_warehouse_user},
                {"step_name": "cutting", "assigned_to": settings.default_cutter_user},
                {"step_name": "printing", "assigned_to": settings.default_printer_user},
                {"step_name": "gluing", "assigned_to": settings.default_finisher_user},
                {"step_name": "drying", "assigned_to": settings.default_finisher_user},
                {"step_name": "packaging", "assigned_to": settings.default_finisher_user},
                {"step_name": "ready", "assigned_to": settings.default_warehouse_user},
            ]
    ...
    def approve(self, request, pk=None):
        ...
        # 3. Generate Production Steps (Dynamic Routing)
        if order.product_template and hasattr(order.product_template, 'routing_steps') and order.product_template.routing_steps.exists():
            # Use Template Routing
            routing = order.product_template.routing_steps.all().order_by('sequence')
            previous_step = None
            
            for r in routing:
                # Map routing step names to ProductionStep choices if needed
                ...
                # Check for specific machine requirement
            ...
            # Fallback: Professional Universal Flow
            # 1. Prepress -> 2. Print -> 3. Lamination (if lacquer) -> 4. Cut -> 5. Glue -> 6. QC -> 7. Pack -> 8. Ready
            
            flow = [
                {'step': 'prepress', 'assign': None}, # Prepress/Design
                {'step': 'printing', 'assign': settings.default_printer_user},
                {'step': 'drying', 'assign': None},
            ]
            
            # Add Lamination if needed
            if order.lacquer_type and order.lacquer_type != 'none':
                flow.append({'step': 'lamination', 'assign': None})
            
            # Add Cutting (Die-cut for boxes, Cutting for others)
...
class ReportsView(APIView):
    ...
    def get(self, request):
        ...
        financials = completed_orders.aggregate(
            total_rev=Sum('total_price'),
            total_expense=Sum('total_cost'),
            count=Count('id')
        )
        
        total_revenue = financials['total_rev'] or 0
        total_cost = financials['total_expense'] or 0
        net_profit = total_revenue - total_cost
        avg_order_value = float(total_revenue / financials['count']) if financials['count'] else 0
        ...
        # 6. Chart Data (Daily Revenue)
        from django.db.models.functions import TruncDate
        daily_stats = completed_orders.annotate(
            day=TruncDate('created_at')
        ).values('day').annotate(
            daily_revenue=Sum('total_price'),
            daily_profit=Sum(F('total_price') - F('total_cost'))
        ).order_by('day')
        ...
        return Response({
            ...
            "charts": {
                "labels": [str(d['day']) for d in daily_stats],
                "revenue": [float(d['daily_revenue'] or 0) for d in daily_stats],
                "profit": [float(d['daily_profit'] or 0) for d in daily_stats]
            },
            "inventory_health": {
...
class DashboardView(APIView):
    ...
    def get(self, request):
        ...
        # Revenue from Completed orders today (or just generally created today? "Bugungi daromad" usually means actual income or sales made)
        # Let's count total_price of orders created today as "Sales Today"
        ...
        # Calculate actual internal profit from finished orders (Price - Cost)
        ...
        order_financials = completed_orders_month.aggregate(
            total_rev=Sum('total_price'),
            total_exp=Sum('total_cost')
        )
        rev = float(order_financials['total_rev'] or 0)
        cost = float(order_financials['total_exp'] or 0)
        internal_profit = rev - cost
        ...
        active_workers = ProductionStep.objects.filter(status='in_progress', assigned_to__isnull=False).values(
            'assigned_to__first_name', 'assigned_to__last_name', 'step', 'order__order_number'
        )

        # 6. CHART DATA (Last 6 months)
        # Simple implementation: Group Orders by month
        # This requires more complex query or helper. Let's do a simple list for last 6 months revenue.
        chart_labels = []
        chart_income = []
        chart_expense = []
        ...
        for i in range(5, -1, -1):
            ...
            # End of month logic skipped for brevity, just using simple chunks or just per month label
            label = m_start.strftime("%b")
            chart_labels.append(label)
            # This is rough, ideally use TruncMonth
            inc = Transaction.objects.filter(type='income', date__year=m_start.year, date__month=m_start.month).aggregate(Sum('amount'))['amount__sum'] or 0
            exp = Transaction.objects.filter(type='expense', date__year=m_start.year, date__month=m_start.month).aggregate(Sum('amount'))['amount__sum'] or 0
        ...
        return Response({
            "stats": {
                ...
                "monthly_internal_profit": internal_profit if request.user.role == 'admin' else None,
