...
class PricingSettings(models.Model):
    ...
    # Phase 6: Advanced Engineering Costs
    knife_price_per_meter = models.DecimalField(
        max_digits=20, 
        decimal_places=2, 
        default=60000, 
        help_text="Pichoqning 1 metri narxi (UZS)"
    )
    base_die_cost = models.DecimalField(
        max_digits=20, 
        decimal_places=2, 
        default=150000, 
        help_text="Bazaviy qolip narxi (Taxta asosi)"
    )
    ...
    setup_waste_sheets = models.IntegerField(default=20, help_text="Stanok sozlash uchun ketadigan varaqlar (dona)")
...
class Client(models.Model):
    ...
    credit_limit = models.DecimalField(
        max_digits=20, 
        decimal_places=2, 
        default=10000000, # 10 mln UZS default
        help_text="Maksimal qarz limiti"
    )
...
class Order(models.Model):
    ...
    priority = models.CharField(max_length=20, choices=PRIORITY_CHOICES, default='normal', help_text="Order priority for scheduling")
    ...
    # Phase 6: Ink Logic
    ink_coverage_percent = models.IntegerField(default=30, help_text="Siyoh bilan qoplanish foizi (0-100)")
    ...
    waste_percent = models.DecimalField(
        ...
        null=True, 
        blank=True,
        help_text="Chiqindi % (Nesting natijasi)"
    )
    layout_description = models.CharField(
        max_length=255, 
        null=True, 
        blank=True,
...
class DesignFile(models.Model):
    ...
    order = models.ForeignKey(Order, on_delete=models.CASCADE, related_name='design_files')
    ...
    approved_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='approved_designs')
    ...
    def __str__(self):
        return f"Design v{self.version} for {self.order.order_number}"

    def approve(self, user):
        """Locks the file and sets approval signature"""
        from django.utils import timezone
        
        # Archive previous approved files
        DesignFile.objects.filter(order=self.order, status='approved').update(status='archived')
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk and not self.version:
            last_version = DesignFile.objects.filter(order=self.order).order_by('-version').first()
            if last_version:
    ...
    def __str__(self):
        return f"Design v{self.version} ({self.order.order_number})"

class OrderGeometry(models.Model):
    """
    Phase 1: Engineering Precision & 3D Data
    Stores parametric data, nesting layouts, and 3D configuration.
    """
    order = models.OneToOneField(Order, on_delete=models.CASCADE, related_name='geometry')
    
    # Template & Dimensions
    template_type = models.CharField(max_length=50, default='mailer_box', help_text="e.g., mailer_box, pizza_box, bag")
    ...
    grain_direction = models.CharField(max_length=20, default='vertical', choices=(('vertical', 'Vertical'), ('horizontal', 'Horizontal')))
    
    # Generated Dieline
    dieline_svg = models.TextField(blank=True, null=True, help_text="Generated SVG path data")
    
    # 3D Visualization
    uv_map_config = models.JSONField(default=dict, blank=True, help_text="Texture mapping config")
    texture_map = models.JSONField(default=dict, blank=True, help_text="User uploaded logo positions")
    
    # Nesting & Production
    nesting_layout = models.JSONField(default=dict, blank=True, help_text="Coordinates on standard sheet")
    
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def __str__(self):
        return f"Geometry for Order {self.order.order_number}"
...
class ProductionStep(models.Model):
    STEP_CHOICES = (
        ...
        ('printing', 'Chop etish'),
        ('drying', 'Quritish'),
        ('lamination', 'Laminatsiya / Lak'),
        ('cutting', 'Kesish'),
        ('die_cutting', 'Shtans (Die-cut)'),
        ('gluing', 'Yelimlash / Buklash'),
        ('qc', 'Sifat Nazorati (QC)'),
        ('packaging', 'Qadoqlash'),
        ('ready', 'Tayyor (Sklad)'),
        ('dispatch', 'Yuklash / Topshirish'),
    )
    STATUS_CHOICES = (
        ('pending', 'Pending'),
...
class MachineSettings(models.Model):
    ...
    # Maintenance Logic
    maintenance_interval_hours = models.IntegerField(default=200, help_text="Moylash/Profilaktika oralig'i (soat)")
    current_operating_hours = models.DecimalField(max_digits=10, decimal_places=2, default=0, help_text="Ishlagan soati")
...
class ProductTemplate(BaseModel):
    ...
    # Phase 1: Engineering Engine
    is_parametric = models.BooleanField(
        default=False,
        help_text="If True, uses Engineering Engine logic (ParametricProfile)"
    )
...
# ========================================
# PHASE 10: PARAMETRIC ENGINEERING ENGINE (Smart Print System Phase 1)
# ========================================

class ParametricProductProfile(BaseModel):
    """
    The 'Engineering DNA' of a product.
    ...
    template = models.OneToOneField(
        'ProductTemplate',
        on_delete=models.CASCADE,
        related_name='parametric_profile',
        help_text="Base template this profile enriches"
    )
    
    # 1. Visualization Logic (Auto-CAD)
    geometry_script = models.JSONField(
        default=dict,
        blank=True,
        help_text="Formula logic for drawing 2D 3D model (e.g. cut_lines, safe_zone)"
    )
    
    # Phase 5: Constructor Fields
    BOX_STYLE_CHOICES = (
        ('custom', 'Custom'),
        ('pizza_box', 'Pizza Box (FEFCO 0427)'),
        ('shopping_bag', 'Shopping Bag'),
    )
    box_style = models.CharField(
        max_length=50, 
        choices=BOX_STYLE_CHOICES, 
        default='custom',
        help_text="Avtomatik konstruktor turi"
    )
    accessories = models.JSONField(
        default=dict, 
        blank=True,
        help_text="Aksessuarlar: handles, eyelets, gluing_points"
    )
    ...
    parts_config = models.JSONField(
        default=list,
        blank=True,
        help_text="List of parts (e.g., [base, lid, insert])"
    )
    
    # 4. Waste Management Logic
    waste_logic_config = models.JSONField(
        default=dict,
        blank=True,
        help_text="Nesting algorithm and inter-item gap rules"
    )
    
    # 5. Ink & Chemical Usage
    ink_density_estimated = models.DecimalField(
        max_digits=5,
        decimal_places=2,
        default=250.0,
        help_text="Estimated ink coverage % (Standard CMYK total)"
    )
    ...
    def __str__(self):
        return f"Engineering Profile: {self.template.name}"
