...
const DynamicBox = ({
    ...
    // Normalize dimensions to Radius roughly for N-sided
    // If sides=4, R should be such that width fits.
    // For 4 sides, Box is W x L. 
    // Let's keep strict Box geometry for sides=4 for backward compat / correctness
    ...
    return (
        <group position={[0, -H / 2, 0]}>
            <PolygonMesh
                ...
                thickness={thickness}
                matProps={matProps}
                foldAngle={foldAngle} // Pass foldAngle
            />
            ...
            {/* LID */}
            {/* For polygon lid, we can just lift it way up if open, or disable */}
            <group position={[0, H + (lidType === 'separate' ? foldAngle / 5 : 0), 0]}>
                {lidType !== 'flat' && (
                    <group position={[0, thickness, 0]}>
                        {/* Pass isLid and maybe 0 foldAngle strictly for lid to keep it rigid? 
                            Or let lid expand? Usually lid is rigid box cap. 
                            Let's keep Lid rigid for now (foldAngle={0}).
                        */}
    ...
    return (
        <div className="w-full h-full min-h-[400px] bg-slate-50 border rounded-lg overflow-hidden relative group">
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur px-6 py-3 rounded-full shadow-lg flex items-center gap-4 transition-all opacity-100 hover:opacity-100">
                <span className="text-xs font-medium text-slate-600">Close</span>
                <input
                    type="range"
                    min="0"
                    max="100"
